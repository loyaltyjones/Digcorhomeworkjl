...
/*
Endpoint:
    Adds a todo to the list. If a list parameter is provided it adds multiple todos in one call
Body:
...
app.post('/todo', (req, res)=> {
    ...
    // if (req.body.includes("a")) {
    //     // code to execute if the string contains 'specific text'
    //     console.log("hello")
    //   } else {
    //      clientError(req, "Invalid data type", 400);
    //      res.status(400).json({error: "Invalid data type"});
    //     // code to execute if the string does not contain 'specific text'
    //   }
...
/*
Endpoint:
    Update an existing todo and modifies its data
Path Parameter:
...
app.delete('/todo/:id', (req, res)=> {
    if (isNaN(Number(req.params.id))) {
      ...
      if (!users && users === undefined) { res.status(400)
        clientError(req, "error: invalid Parameters ID", 400)
        res.status(400).json({error:"Invalid Parameters ID"});
      }
    }
